...
-- Function to create a telescope input box
---@param prompt string
...
local function telescope_input(prompt, callback)
  ...
  input_box:on(nui_utils_event.BufLeave, function() input_box:unmount() end)
end
...
local function async_http_request(opts)
  ...
  if databricks_token then headers["Authorization"] = "Bearer " .. databricks_token end
  ...
  curl.request({
    ...
    on_error = function(err) print("HTTP Request Error: " .. vim.inspect(err)) end,
  })
end
  ...
  async_http_request({
    ...
    callback = function(data)
      ...
      if os.getenv("MLFLOW_TRACKING_URI") == "databricks" then
        run_uri = mlflow_uri
          .. "/ml/experiments/"
          .. run_data.run.info.experiment_id
          .. "/runs/"
          .. run_data.run.info.run_id
      else
        run_uri = mlflow_uri
          .. "/#/experiments/"
          .. run_data.run.info.experiment_id
...
local function bring_run_metrics()
  telescope_input("Enter run id", function(run_id) fetch_run_metrics(run_id, paste_run_metrics) end)
end
...
M.setup = function(opts)
  ...
  if mlflow_uri == "databricks" then
    ...
    if not databricks_host or not databricks_token then
      error(
        "When using Databricks MLflow, both DATABRICKS_HOST and DATABRICKS_TOKEN environment variables are required"
      )
    end
